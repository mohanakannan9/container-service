...
import org.nrg.actions.daos.ResolvedCommandDao;
...
@Service
public class HibernateResolvedCommandService
        extends AbstractHibernateEntityService<ResolvedCommand, ResolvedCommandDao>
        implements ResolvedCommandService {
    @Autowired
    private CommandService commandService;

    @Override
    public ResolvedCommand resolveCommand(final ActionContextExecution ace) throws NotFoundException {
        final Command command = commandService.retrieve(ace.getCommandId());
        if (command == null) {
            throw new NotFoundException("Could not find Command with id " + ace.getCommandId());
        }
        ...
        if (DockerImageCommand.class.isAssignableFrom(command.getClass())) {
            ...
            throw new NotFoundException("Cannot find docker image id for command " + ace.getCommandId());
        }
